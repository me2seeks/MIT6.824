...
func mapTask(task *MapTask, mapf func(string, string) []KeyValue) {
	...
	for i, ikva := range omap {
		...
		if err != nil {
			log.Fatalf("cannot create tmp %v", err)
		}
		encoder := json.NewEncoder(tmpFile)
		for _, kv := range ikva {
			if err := encoder.Encode(&kv); err != nil {
				log.Fatalf("cannot encoder %v", err)
			}
		}
		tmpFile.Close()
...
func reduceTask(task *ReduceTask, reducef func(string, []string) string) {
	...
	tmpfile.Close()
	if err := os.Rename(tmpfile.Name(), fmt.Sprintf("mr-out-%d", task.Id)); err != nil {
		log.Fatal(err)
	}
